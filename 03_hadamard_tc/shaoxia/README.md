...
| `hadacore.cu` | 纯 Tensor Core kernel（原 torch 绑定版，已改为纯 CUDA） |
...
| `Makefile` | 构建脚本 |
| `bench.sh` | 运行 CUDA Hadamard kernel 的便捷脚本 |
...
| 参数 | 默认 | 说明 |
| --- | --- | --- |
| `--impl` | `2` | 自研内核实现：`1`(scalar) `2`(向量化) `3`(multi-warp) `4`(auto) `hybrid`(MMA/CUDA Core 混合) |
...
1. **correctness** —— 与 fp32 FWHT 参考值对比，并校验「自研 vs fht」与「hadacore vs fht」的最大误差。两者均严格使用 `≤ 1e-2`（fp16）/ `≤ 5e-2`（bf16）的项目门槛。
...
- 自研 `hadamard.cu` 各 impl 的 `cols` 支持范围不同，`4` 支持 `2..32768`。
